 ...
 * 顶层的节点被称为根节点，根节点下面可以包含树枝节点和叶子节点，
 ...
 * 透明式：抽象构件声明了所有子类中的全部方法，所以客户端无须区别树叶对象和树枝对象，
 * 对客户端来说是透明的。但其缺点是：树叶构件本来没有 Add()、Remove() 及 GetChild() 方法，
 * 却要实现它们（空实现或抛异常），这样会带来一些安全性问题。
 * 安全式：将管理子构件的方法移到树枝构件中，抽象构件和树叶构件没有对子对象的管理方法，这样就避免了上一种方式的安全性问题，
